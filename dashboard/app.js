...
const consumer = new kafka.Consumer(client, [{topic: 'access-logs-sink'}], {autoCommit: false});

const toSchema = (obj) => {
    if ("browser_categories" in obj) {
        return {
            key: "browser_categories",
            data: {[obj.browser_categories.accessed_by]: obj.browser_categories.count}
        }
    }
    if ("types_of_vendors" in obj) {
        return {
            key: "types_of_vendors",
            data: {[obj.types_of_vendors.browser_vendor]: obj.types_of_vendors.count}
        }
    }
    if ("types_of_operating_systems" in obj) {
        return {
            key: "types_of_operating_systems",
            data: {[obj.types_of_operating_systems.os]: obj.types_of_operating_systems.count}
        }
    }
